...
@router.get("/", response_model=List[CommentOpt], dependencies=[Depends(RateLimiter(times=25, seconds=60))])
@cache(expire=settings.cache_expire_http_responce)
async def list_comment():
    return await service.list()

@router.get("/{id}", response_model=CommentOpt, dependencies=[Depends(RateLimiter(times=25, seconds=60))])
@cache(expire=settings.cache_expire_http_responce)
async def get_comment_by_id(id: int):
    return await service.get_by_id(id)

@router.post("/", response_model=CommentOpt, dependencies=[Depends(RateLimiter(times=25, seconds=60))])
...
@router.put("/{id}", response_model=CommentOpt, dependencies=[Depends(RateLimiter(times=25, seconds=60))])
...
@router.delete("/{id}", response_model=CommentOpt, dependencies=[Depends(RateLimiter(times=25, seconds=60))])
